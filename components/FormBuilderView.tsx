...
type FieldType = 'text' | 'number' | 'select' | 'date' | 'file' | 'textarea' | 'toggle' | 'radio' | 'divider' | 'breakpoint';
...
const INITIAL_FORMS: CustomForm[] = [
  {
    ...
    fields: [
      ...
      { id: 'fd-div', type: 'divider', label: 'Financials', placeholder: '', required: false },
...
const FormBuilderView: React.FC = () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {forms.map(form => (
            <div key={form.id} onClick={() => setSelectedForm(form)} className="bg-white dark:bg-surface-dark border border-slate-100 dark:border-slate-800 p-6 rounded-[24px] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group">
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">{form.category} • {form.fields.length} Fields</p>
  ...
  const currentCategories = CATEGORIES[selectedForm.type];
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                 ...
                 <select 
                   ...
                   onChange={(e) => {
                      const newType = e.target.value as 'Loan' | 'Investment';
                      const updated: CustomForm = { 
                        ...selectedForm, 
                        type: newType,
                        category: CATEGORIES[newType][0] // Reset to first category of new type
                      };
                      setSelectedForm(updated);
                      setForms(forms.map(f => f.id === updated.id ? updated : f));
                   }}
              ...
              <div className="flex items-center gap-3">
                 ...
                 <select 
                   ...
                   onChange={(e) => {
                      const updated = { ...selectedForm, category: e.target.value };
                      setSelectedForm(updated);
                      setForms(forms.map(f => f.id === updated.id ? updated : f));
                   }}
                   className="bg-primary/5 border-none text-[10px] font-black text-primary uppercase p-0 focus:ring-0 cursor-pointer"
          ...
          <div className="bg-white dark:bg-surface-dark rounded-[32px] p-8 border border-slate-100 dark:border-slate-800 shadow-sm min-h-[600px]">
            <div className="mb-10 text-center">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 rounded-full mb-3">
                <span className="text-[9px] font-black text-primary uppercase tracking-widest">{selectedForm.type} &bull; {selectedForm.category}</span>
              </div>
              <h4 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">{selectedForm.name}</h4>
              <p className="text-slate-400 text-sm font-bold mt-1">Please provide the following information to proceed with your application.</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              {selectedForm.fields.map((field, idx) => {
                const isStructural = field.type === 'divider' || field.type === 'breakpoint';
                
                return (
                  <div 
                    key={field.id}
                    onClick={() => !isPreview && setActiveField(field.id)}
                    className={`relative rounded-[24px] transition-all group ${
                      isStructural ? 'py-4' : 'p-6 border-2'
                    } ${
                      isPreview 
                        ? 'border-transparent bg-slate-50 dark:bg-surface-darker' 
                        : activeField === field.id 
                        ? 'border-primary bg-primary/5' 
                        : isStructural ? '' : 'border-slate-50 dark:border-slate-800/50 hover:border-slate-200 cursor-pointer'
                    }`}
                  >
                    <div className="space-y-3">
                      {field.type === 'divider' ? (
                        <div className="flex items-center gap-4 py-2">
                           <span className="text-[10px] font-black text-primary uppercase tracking-widest whitespace-nowrap">{field.label}</span>
                           <div className="h-px w-full bg-slate-100 dark:bg-slate-800" />
                        </div>
                      ) : field.type === 'breakpoint' ? (
                        <div className="bg-slate-900 dark:bg-slate-800 py-3 px-6 rounded-xl flex items-center justify-between">
                           <span className="text-xs font-black text-white uppercase tracking-[0.2em]">{field.label || 'Next Section'}</span>
                        ...
                        <>
                          ...
                          {field.type === 'textarea' ? (
                            ...
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-6 rounded-full bg-slate-200 dark:bg-slate-700 relative">
                                <div className="absolute top-1 left-1 w-4 h-4 rounded-full bg-white shadow-sm" />
                              </div>
                              <span className="text-xs font-bold text-slate-500 italic">Toggle Switch</span>
                            </div>
                          ) : field.type === 'radio' ? (
                    ...
                    {!isPreview && (
                      <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white dark:bg-surface-dark p-1 rounded-lg shadow-xl border border-slate-100 dark:border-slate-800 z-10">
              ...
              {currentField ? (
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                        {currentField.type === 'divider' || currentField.type === 'breakpoint' ? 'Label / Heading' : 'Input Label'}
                      </label>
                      <input 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Field UI Type</label>
                      <select 
                      ...
                      >
                        ...
                        <optgroup label="Layout Elements">
                          ...
                          <option value="breakpoint">Page Breakpoint</option>
                        </optgroup>
                      </select>
                    </div>

                    {(currentField.type !== 'file' && currentField.type !== 'date' && currentField.type !== 'divider' && currentField.type !== 'breakpoint' && currentField.type !== 'toggle' && currentField.type !== 'radio') && (
                      <div className="space-y-1.5">
                    ...
                    {(currentField.type !== 'divider' && currentField.type !== 'breakpoint') && (
                      <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-background-dark/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                        <div className="space-y-0.5">
                           ...
                           <p className="text-[10px] text-slate-400 font-bold">Mandatory for submission</p>
                        </div>
                        <button 
                    ...
                    {(currentField.type === 'select' || currentField.type === 'radio') && (
                       <div className="space-y-2 pt-2 animate-in slide-in-from-top-2">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Manage Options</label>
                         <div className="space-y-2">
                           {(currentField.options || ['Option 1', 'Option 2']).map((opt, oIdx) => (
                             <div key={oIdx} className="flex gap-2">
                                <input 
                                  value={opt}
                                  onChange={(e) => {
                                    const opts = [...(currentField.options || ['Option 1', 'Option 2'])];
                                    opts[oIdx] = e.target.value;
                                    updateField(currentField.id, { options: opts });
                                  }}
                                  className="flex-1 bg-slate-50 dark:bg-background-dark/50 border border-slate-100 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-bold focus:ring-1 focus:ring-primary dark:text-white"
                                />
                                <button 
                                  onClick={() => {
                                    const opts = (currentField.options || ['Option 1', 'Option 2']).filter((_, idx) => idx !== oIdx);
                                    updateField(currentField.id, { options: opts });
                                  }}
                         ...
                         <button 
                           onClick={() => updateField(currentField.id, { options: [...(currentField.options || ['Option 1', 'Option 2']), 'New Option'] })}
                           className="w-full mt-2 py-2 border border-dashed border-primary/30 rounded-xl text-[10px] font-black text-primary uppercase tracking-widest hover:bg-primary/5 transition-all"
                         >
                           + Add New Option
                         </button>
                       </div>
                ...
                <div className="text-center py-20 space-y-4">
                  ...
                  <div className="space-y-1">
                    <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Canvas Inspector</p>
                    <p className="text-xs text-slate-500 font-bold px-6">Select a field on the layout to modify its logic, validation, or appearance.</p>
                  </div>
                </div>
              )}
            </div>
